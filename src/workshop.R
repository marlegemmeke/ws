...
setwd("C:/Users/marle/Documents/workshop")

DBI.180.FRIDS2 <- read.csv2("~/workshop/data/processed/FAKE.csv")


############ baseline characteristics

DBI.180.FRIDS2 %>% group_by(DBI.3cat) %>% summarise(count=n())
subset(DBI.180.FRIDS2, DBI.180.FRIDS2$val =="1") %>% group_by(DBI.3cat) %>% summarise(count=n())
...
model.180.DBI <- multinom(as.factor(val) ~ DBI.3cat +leeftijd+ geslacht , data=DBI.180.FRIDS2, Hess=TRUE)

summary(model.180.DBI)
exp(coef(model.180.DBI))
exp(confint(model.180.DBI))


model180.FRIDS <- multinom(as.factor(val) ~  leeftijd + geslacht + SSRI + TCA + benzodiazepines
...
summary(model.180.FRIDS)
exp(coef(model.180.FRIDS))
...
source("~/workshop/src/VanHoorde_functions.R")
source("~/workshop/src/VanHoorde_functions.R")
)
